...
def parse_nodes(arch,assembly_functions,cfg):
    ...
    for func_addr,func in assembly_functions.items():
        ...
        for i in range(len(func.instr_list)):
            ...
            call_to_trampoline = (func.instr_list[i].instr in arch.call_instrs and arch.trampoline in func.instr_list[i].arg)
            if not call_to_trampoline and (ret_via_pop or func.instr_list[i].instr in br_instrs):
    ...
    for node_addr,node in cfg.nodes.items():
        if node.type == "cond":
            if arch == "elf32-msp430":
                ...
                node.add_successor('0x'+node.instr_addrs[-1].arg.split(' ')[0])
            if node.adj_instr:
...
def create_cfg(arch, lines):
    ...
    for key in cfg.nodes.keys():
        print("key = "+str(key), file=debugFile)
        for inst in cfg.nodes[key].instr_addrs:
            print("\t"+str(inst), file=debugFile)
    debugFile.close()
